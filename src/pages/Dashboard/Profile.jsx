...
const Profile = () => {
  ...
  const isOwnProfile = !email || email === dbUser?.email;

  const fetchProfileData = async () => {
    const targetEmail = email || dbUser?.email;
    if (!targetEmail) return;

    try {
      setLoading(true);
      const [userRes, statsRes, lessonsRes] = await Promise.all([
        api.get(`/users/${targetEmail}`),
        api.get(`/stats/user/${targetEmail}`),
        api.get(`/lessons?authorEmail=${targetEmail}&limit=100`),
      ]);
    ...
    <div className="space-y-12 pt-10 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="relative flex flex-col md:flex-row md:items-end gap-6 -mt-12 mb-8">
                    ...
                    <p className="text-gray-500 font-medium flex items-center gap-2">
                        <FaEnvelope className="text-sm" /> {profileUser?.email}
                    </p>
                    </div>
                    {isOwnProfile && (
                    <button
            ...
            <div className="space-y-8 pt-10">
                ...
                <h2 className="text-2xl font-bold text-gray-900">Shared Lesson</h2>
